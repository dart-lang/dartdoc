...
class CoverageSubprocessLauncher extends SubprocessLauncher {
  ...
  String get outCoverageFilename => _outCoverageFilename ??= pathLib.join(tempDir.path, 'coverage.out.${observatoryPort}');
  ...
  static Future<void> generateCoverageToFile(File outputFile) async {
    ...
    return launcher.runStreamed('pub', ['run', 'coverage:format_coverage',
      ...
      '--report-on=bin,lib',
      '-i'
    ]..addAll(currentCoverageResults.map((t) => t.item1)));
  }

  @override
  Future<Iterable<Map>> runStreamed(String executable, List<String> arguments, {String workingDirectory}) {
    assert(executable == Platform.executable, 'Must use dart executable for tracking coverage');

    if (coverageEnabled) {
      arguments = ['--enable-vm-service=${observatoryPort}',
      '--pause-isolates-on-exit']..addAll(arguments);
    }

    Future<Iterable<Map>> results = super.runStreamed(executable, arguments, workingDirectory: workingDirectory);

    if (coverageEnabled) {
      coverageResults.add(new Tuple2(outCoverageFilename,
        super.runStreamed('pub', ['run', 'coverage:collect_coverage',
          '--wait-paused',
          '--resume-isolates',
