...
class DartdocExperimentOption extends DartdocOption<bool> {
  ...
  DartdocExperimentOption(String name, bool defaultsTo,
      {String help = '', bool expired = false, bool hide = false})
      : this.expired = expired,
        this.hide = hide,
        super(name, defaultsTo, help, false, false, false, null);
  ...
  bool valueAt(Directory dir) {
    ...
    if (expired && value != defaultsTo) {
      throw new DartdocOptionError(
          'Experiment ${name} can not be set to ${value} -- experiment has expired');
    }
    return value;
  }
}
...
abstract class DartdocExperimentOptionContext
  ...
  bool get experimentConstantUpdate2018 =>
      optionSet['enable-experiment']['constant-update-2018'].valueAt(context);
  bool get experimentNonNullable =>
      optionSet['enable-experiment']['non-nullable'].valueAt(context);
  bool get experimentSetLiterals =>
      optionSet['enable-experiment']['set-literals'].valueAt(context);
}
...
Future<List<DartdocOption>> createExperimentOptions() async {
  List<DartdocExperimentOption> experiments = [
    new DartdocExperimentOption('constant-update-2018', false,
        help: 'Q4 2018 Constant Update'),
    ...
    new DartdocOptionArgFile<List<String>>('enable-experiment', [],
        help: 'Enable or disable listed experiments.\n' +
            experiments
                .where((e) => !e.hide)
                .map((e) =>
                    '    [no-]${e.name}: ${e.help} (default: ${e.defaultsTo})')
                .join('\n'))
      ..addAll(experiments),
  ];
}
