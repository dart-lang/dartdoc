...
final Directory flutterDirDevTools =
    Directory(path.join(_flutterDir.path, 'dev', 'tools'));
...
Future<String> createComparisonDartdoc() async {
  ...
  await launcher.runStreamed(sdkDart, ['pub', 'get'],
      workingDirectory: dartdocClean.path);
...
Future<void> testWithAnalyzerSdk() async {
  ...
  try {
    await launcher.runStreamed(
        sdkDart, ['pub', 'run', 'grinder', defaultGrindParameter],
...
Future<Iterable<Map<String, Object?>>> _buildSdkDocs(
  ...
  await launcher.runStreamed(sdkDart, ['pub', 'get'], workingDirectory: cwd);
...
Future<Iterable<Map<String, Object?>>> _buildTestPackageDocs(
  ...
  var testPackagePubGet = launcher.runStreamed(sdkDart, ['pub', 'get'],
      workingDirectory: testPackagePath);
  var dartdocPubGet =
      launcher.runStreamed(sdkDart, ['pub', 'get'], workingDirectory: cwd);
...
Future<void> startTestPackageDocsServer() async {
  ...
  await launcher.runStreamed(sdkDart, [
    'pub',
    'global',
    'run',
    'dhttpd',
...
Future<void> _serveDocsFrom(String servePath, int port, String context) async {
  ...
  if (!_serveReady) {
    await launcher.runStreamed(sdkDart, ['pub', 'get']);
    await launcher
        .runStreamed(sdkDart, ['pub', 'global', 'activate', 'dhttpd']);
    _serveReady = true;
  }
  await launcher.runStreamed(sdkDart, [
    'pub',
...
Future<void> compareFlutterWarnings() async {
  ...
  if (Platform.environment['SERVE_FLUTTER'] == '1') {
    ...
    await launcher.runStreamed(sdkDart, ['pub', 'get']);
    var original = launcher.runStreamed(sdkDart, [
      'pub',
      'global',
      'run',
      'dhttpd',
    ...
    var current = launcher.runStreamed(sdkDart, [
      'pub',
      'global',
      'run',
      'dhttpd',
      ...
      '9001',
      '--path',
      path.join(_flutterDir.absolute.path, 'dev', 'docs', 'doc'),
    ]);
...
Future<void> serveFlutterDocs() async {
  ...
  await launcher.runStreamed(sdkDart, [
    ...
    '8001',
    '--path',
    path.join(_flutterDir.path, 'dev', 'docs', 'doc'),
  ]);
}
...
Future<void> serveLanguageTestDocs() async {
  ...
  await launcher.runStreamed(sdkDart, [
    ...
    '8004',
    '--path',
    path.join(_languageTestPackageDir.path, 'doc', 'api'),
  ]);
}
...
Future<void> buildLanguageTestDocs() async {
  ...
  var pubspecFile =
      File(path.join(_languageTestPackageDir.path, 'pubspec.yaml'));
...
''');
  ...
  var analyzerOptionsFile =
      File(path.join(_languageTestPackageDir.path, 'analysis_options.yaml'));
...
$analyzerOptions
 ''');
  ...
  var libDir = Directory(path.join(_languageTestPackageDir.path, 'lib'))
    ..createSync();
  var languageTestDir =
...
Future<void> tryPublish() async {
  ...
  await launcher.runStreamed(sdkDart, ['pub', 'publish', '-n']);
}
...
String get sdkDart => sdkBin('dart');
...
Future<void> testDartdoc() async {
  ...
  expectFileContains(path.join(_dartdocDocsPath, 'index.html'),
      ['<title>dartdoc - Dart API docs</title>']);
  var object = RegExp('<li>Object</li>', multiLine: true);
  expectFileContains(
      path.join(_dartdocDocsPath, 'dartdoc', 'ModelElement-class.html'),
      [object]);
}
...
Future<void> testDartdocFlutterPlugin() async {
  ...
  expectFileContains(
      path.join(
          _pluginPackageDocsPath, 'testlib', 'MyAwesomeWidget-class.html'),
      [
...
void validateSdkDocs() {
  ...
  var futureConstFile =
      joinFile(_sdkDocsDir, [path.join('dart-async', 'Future', 'Future.html')]);
