...
final RegExp observatoryPortRegexp = new RegExp(r'^Observatory listening on http://.*:(\d+)');
...
class CoverageSubprocessLauncher extends SubprocessLauncher {
  ...
  static bool get coverageEnabled => Platform.environment.containsKey('COVERAGE_TOKEN');
  ...
  static String buildNextCoverageFilename() => pathLib.join(tempDir.path, 'dart-cov-${pid}-${nextRun++}.json');
  ...
  Future<Iterable<Map>> runStreamed(String executable, List<String> arguments,
      {String workingDirectory, Map<String, String> environment, bool includeParentEnvironment = true, void Function(String) perLine}) async {
    assert(executable == Platform.executable || executable == Platform.resolvedExecutable, 'Must use dart executable for tracking coverage');
    ...
    Future<Iterable<Map>> results = super
        .runStreamed(executable, arguments, environment: environment, includeParentEnvironment: includeParentEnvironment, workingDirectory: workingDirectory, perLine: parsePortAsString);
...
class SubprocessLauncher {
  ...
  Future<Iterable<Map>> runStreamed(String executable, List<String> arguments,
    ...
    Process process = await Process.start(realExecutable, realArguments,
        workingDirectory: workingDirectory, environment: environment, includeParentEnvironment: includeParentEnvironment);
